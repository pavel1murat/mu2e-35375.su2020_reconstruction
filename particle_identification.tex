...
PID ANN training performed using ROOT TMVA package uses 5000 electron and 5000 muon events.
...
Figure \ref{fig:pid_score_mumep} shows distributions of the PID score  for 92 MeV positrons and $\mu^+$'s. 
...
\hspace{-0.6in}
  \begin{tikzpicture}
    \node[anchor=south west,inner sep=0] at (0,0.) {
      % \node[shift={(0 cm,0.cm)},inner sep=0,rotate={90}] at (0,0) {}
      % \makebox[\textwidth][c] {
      \includegraphics[width=0.60\textwidth]{figures/pdf/missing_plot}
      % }
    };
  \end{tikzpicture}
  \captionof{figure} {
    \label{fig:pid_score_mumep}
    Quality of the PID ANN training for\MuToEp\ channel: PID score distributions for 92.3 positrons and positive muons
  }
\end{figure}